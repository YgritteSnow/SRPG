...
#ifdef _DEBUG
#include "IMGUIVertexShader.hlsl.Debug.pcsh"
#include "IMGUIPixelShader.hlsl.Debug.pcsh"
#else
...
struct IMGUISystemD3D11Impl : public IMGUISystemD3D11
{
	// Data
	INT64                    g_Time = 0;
	...
	bool    ImGui_ImplDX11_CreateDeviceObjects()
	{
		...
		// Create the vertex shader
		{
			...
			if (g_pd3dDevice->CreateInputLayout(local_layout, 3, CompiledShaderCode_IMGUIVertexShader_main, ArraySize(CompiledShaderCode_IMGUIVertexShader_main), &g_pInputLayout) != S_OK)
				return false;
	...
	virtual bool    ImGui_ImplDX11_Init(void* hwnd, ID3D11Device* device, ID3D11DeviceContext* device_context) override
	{
		g_hWnd = (HWND)hwnd;
		g_pd3dDevice = device;
	...
	virtual void ImGui_ImplDX11_Shutdown() override
	{
		ImGui_ImplDX11_InvalidateDeviceObjects();
		ImGui::Shutdown();
		g_pd3dDevice = NULL;
	...
	virtual void ImGui_ImplDX11_NewFrame() override
	{
		if (!g_pFontSampler)
	...
	virtual void* GetWndProcHandler() override
	{
		return &ImGui_ImplDX11_WndProcHandler;
	}
};

Ptr<IMGUISystemD3D11> IMGUISystemD3D11::Create()
{
